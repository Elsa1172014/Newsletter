...
module.exports = async function handler(req,res){
  ...
  try{
    const prefix='arabicmag:issue:';
    let cursor='0';
    const keys=[];
    do{
      const result = await cmd(url,token,['SCAN',cursor,'MATCH',prefix+'*','COUNT','200']);
    ...
    for(const key of keys){
      try{
        ...
        discovered.push({
          id: issue.id || key.slice(prefix.length),
          key,
          title: issue?.meta?.title || '',
          week: issue?.meta?.week || '',
    ...
    let existing=[];
    try{
      const raw = await cmd(url,token,['GET','arabicmag:index']);
    ...
    for(const id of foundIds) if(!repaired.includes(id)) repaired.push(id);
    if(recoveredIds.length){
      await cmd(url,token,['SET','arabicmag:index',JSON.stringify(repaired)]);
    }

    discovered.sort((a,b)=> String(b.createdAt).localeCompare(String(a.createdAt)));
    res.status(200).json({ok:true, existingCount:existing.length, discoveredCount:discovered.length, recoveredCount:recoveredIds.length, recoveredIds, issues:discovered});
  }catch(err){
