...
const GEMINI_MODEL = "gemini-3.6-flash"; // غيّر هذا السطر فقط إن رغبت باستخدام موديل Gemini آخر لاحقًا
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!text) {
      const finishReason = data?.candidates?.[0]?.finishReason || 'غير معروف';
      res.status(502).json({ error: `استجابة Gemini لم تحتوِ على نص (finishReason: ${finishReason}) — قد يكون المحتوى محجوبًا بواسطة فلاتر الأمان، أو نفد حد التوكنز` });
      return;
    }
